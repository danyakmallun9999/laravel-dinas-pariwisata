        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden relative">
                <div class="absolute top-0 right-0 p-8 opacity-5">
                     <i class="fa-solid fa-user-pen text-9xl text-indigo-600 transform rotate-12"></i>
                </div>

                <div class="p-8 md:p-10 relative z-10">
                    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-8" 
                          x-data="{ role: '{{ $user->roles->first()?->name ?? '' }}' }">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="name" class="block mt-2 w-full rounded-xl border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 py-3" type="text" name="name" :value="old('name', $user->name)" required autofocus autocomplete="name" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="email" class="block mt-2 w-full rounded-xl border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 py-3" type="email" name="email" :value="old('email', $user->email)" required autocomplete="username" />
                        ...
                        <div class="bg-gray-50/50 rounded-2xl p-6 border border-gray-100 space-y-6">
                             <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs text-gray-500 bg-white px-3 py-1 rounded-full border border-gray-200">Opsional</span>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <!-- Password -->
                                <div>
                                    <x-input-label for="password" :value="__('Password Baru')" class="text-gray-600 font-medium" />
                                ...
                                <!-- Confirm Password -->
                                <div>
                                    <x-input-label for="password_confirmation" :value="__('Konfirmasi Password Baru')" class="text-gray-600 font-medium" />
                            ...
                            <div>
                                ...
                                <div class="mt-2 relative">
                                    <select id="role" name="role" x-model="role" class="block w-full rounded-xl border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 py-3 pl-4 pr-10 appearance-none bg-no-repeat bg-right-4">
                                        <option value="" disabled>-- Pilih Peran Admin --</option>
                                        @foreach($roles as $roleOption)
                                            <option value="{{ $roleOption->name }}" {{ ($user->roles->first()?->name === $roleOption->name) ? 'selected' : '' }}>
                                        ...
                                        <select id="place_id" name="place_id" class="block mt-2 w-full rounded-xl border-blue-200 focus:border-blue-500 focus:ring-blue-500/20 py-3">
                                            ...
                                            @foreach($places as $place)
                                                <option value="{{ $place->id }}" 
                                                    {{ ($user->ownedPlaces->contains($place->id)) ? 'selected' : '' }}>
                                                    {{ $place->name }}
                                                </option>
                                            @endforeach
                                        </select>
